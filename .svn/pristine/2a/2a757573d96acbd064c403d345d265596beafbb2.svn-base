...
public class SB {
	...
	public static String ROOT = "sillybird";//默认工作目录
	...
	/** 设置SD卡上的工作目录, setSDRoot("/根目录名/")，前面不要加sd卡前缀，都有了，访问：SD.ROOT，自动带了最后的斜杠  */
	public static void setSDRoot(String path){
		if(SB.common.isEmpty(path)) path = "sillybird";
		SB.ROOT = Environment.getExternalStorageDirectory().getAbsolutePath() + path;
		if(!SB.ROOT.endsWith("/")){
			SB.ROOT += "/";
		}
		SB.fileop.createDirIfNotExist(SB.ROOT);
	}
		...
		public static String getPathInRoot(String subPath){
			if(subPath != null){
				if(subPath.startsWith("/")){
					return SB.ROOT + subPath.substring(1);
				}else{
					return SB.ROOT + subPath;
				}
			}
			return SB.ROOT;
		}
		/** 获得工作目录下的指定路径，不能以/开头，例如aa/rr.txt，返回/storage/工作目录/aa/rr.txt */
		public static File getFileInRoot(String subPath){
			return new File(getPathInRoot(subPath));
		}
...
//		}
	...
	public static class collection{
		...
		public static void walk(Collection<?> c, CommonCallback callback){
			if(callback == null) return;
			if(SB.common.isNotEmpty(c)){
				for(Object o: c){
					callback.process(o);
				}
			}
	...
	public static class display{
		...
		public static  int dip2px(float dpValue) {  
	        final float scale = SB.context.getResources().getDisplayMetrics().density;  
